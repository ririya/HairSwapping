...
Mat synthesizeSkin(Mat imgRGB, Face face, Mat hairMask)
{
	...
	/*	facePixels.at<Vec3b>(i, firstJ)[0] = 0;
		...
		if (firstJ < middleInd)   // if first column less than middle column, ignore left side of face
		{
			...
			updateForeheadPixels(i, firstL, lastL, currfirstJ, currlastJ, numberOfPoints, colorEst.getAvgSkinColor(), facePixels_Lab);
			
			//updateForeheadPixelsWithFixedValue(i, firstL, lastL, currfirstJ, currlastJ, colorEst.getAvgSkinColor(), facePixels_Lab, firstL);
		}
...
void updateForeheadPixelsWithFixedValue(int i, uchar firstL, uchar lastL, int firstJ, int lastJ, Scalar avgSkinColor, Mat facePixels_Lab, uchar fixedValue)
{
	...
	{
		facePixels_Lab.at<Vec3b>(i, j)[0] = fixedValue;
