...
int extractHair(Mat img, Face face, Mat *labels, Hair *hair)
{
	...
	*labels = getLabelsImage(labelsSequence, img.rows, img.cols);
	...
	if (USE_MATTING)
	{
		hairImageMask = performMatting(hairImageMask, img);
	}

	/*Mat hairPixels(img.rows, img.cols, CV_8UC3, Scalar(255, 255, 255));
	img.copyTo(hairPixels, hairImageMask);

	cv::imshow("hairImageMask", hairPixels);
	cv::waitKey();*/

	*hair = findConnectionPoint(hairImageMask, face);
...
Hair findConnectionPoint(Mat hairMask, Face face)
{
	...
	return Hair(hairMask, hairConnectionPointLocationX, hairConnectionPointLocationY, hairConnectionPointDistanceToJ_X, hairConnectionPointDistanceToJ_Y);
}
...
Mat performMatting(Mat hairImageMask, Mat Image)
{
	...
	dilate(hairImageMaskWhite, hairImageMaskGray, element);
	...
	threshold(alpha, alpha, 0.0, 1.0, cv::THRESH_BINARY);
